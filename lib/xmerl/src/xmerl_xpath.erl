...
string(Str, Node, Parents, Doc, Options) ->
    ...
    Context = 
	(new_context(Options))#xmlContext{context_node = ContextNode,
					  whole_document = WholeDoc},
    NewContext = match(Str, #state{context = Context}),
...
new_context([{namespace, NS = #xmlNamespace{nodes = Nodes}}|T], C) ->
...
full_parents1([{Name, Pos}|Ns], Content, Parents) ->
    ...
    PN = #xmlNode{type = element,
		  node = E,
		  parents = Parents},
    full_parents1(Ns, E#xmlElement.content, [PN|Parents]);
full_parents1([], E, Parents) ->
    Parents.


locate_element(Name, Pos, [E = #xmlElement{name = Name, pos = Pos}|_]) ->
    E;
locate_element(Name, Pos, [#xmlElement{pos = P}|_]) when P >= Pos ->
    %% we've passed Pos (P > Pos) or the name is wrong (P == Pos)
    exit(invalid_parents);
locate_element(Name, Pos, []) ->
    exit(invalid_parents);
locate_element(Name, Pos, [H|T]) ->
    locate_element(Name, Pos, T).


match(Str, S = #state{context = C}) ->
...
eval_path(union, {PathExpr1, PathExpr2}, C = #xmlContext{}) ->
    ...
    S2 = match_expr(PathExpr2, S1#state{acc = NewNodeSet}),
    S2#state.context;
eval_path(abs, PathExpr, C = #xmlContext{}) ->
    ...
    #state{context = NewContext} = path_expr(PathExpr, S),
    NewContext;
...
eval_path(filter, {PathExpr, PredExpr}, C = #xmlContext{}) ->
    ...
    #state{context = NewContext} = pred_expr(PredExpr, S1),
    NewContext.




path_expr(E = {refine, StepExpr1, StepExpr2}, S) ->
    ?dbg("E = ~p~n", [E]),
...
path_expr(E = {step, {Axis, NodeTest, PredExpr}}, S = #state{context = C,
							     acc = Acc}) ->
    ?dbg("E = ~p~n", [E]),
...
pred_expr([], S) ->
    S;
pred_expr(E= [{pred, Pred}|Preds], S = #state{context = C}) ->
    ?dbg("E = ~p~n", [E]),
...
axis(Axis, NodeTest, Context) ->
    axis(Axis, NodeTest, Context, Acc = []).
...
match_descendant(Tok, N, Acc, Context) ->
	...
	element ->
	    NewPs = [N|Ps],
	    match_desc(Node#xmlElement.content, NewPs, Tok, Acc, Context);
	Other ->
	    Acc
    end.
...
match_desc([E = #xmlElement{content = C}|T], Parents, Tok, Acc, Context) ->
    ...
    Acc2 = match_desc(C, NewParents, Tok, Acc1, Context),
...
match_desc([], Parents, Tok, Acc, Context) ->
    Acc.
...
match_child(Tok, N, Acc, Context) ->
	...
	element ->
	    ...
	    lists:foldl(
	      ...
	      end, Acc, Node#xmlElement.content);
	Other ->
	    Acc
    end.
...
match_following_sibling(Tok, #xmlAttribute{}, Acc, Context) ->
    Acc;
match_following_sibling(Tok, #xmlNamespace{}, Acc, Context) ->
    Acc;
...
match_following_sibling(Tok, N, Acc, Context) ->
	...
	[#xmlNode{type = element,
		  node = #xmlElement{content = PContent}}|_] ->
	...
	[#xmlNode{type = element,
	    ...
	    lists:foldl(
	      fun(E, AccX) ->
		      ...
		      match_desc(E#xmlElement.content, Tok, Ps, Acc1, Context)
	      end, Acc, FollowingSiblings);
	Other ->
	    Acc
    end.
...
match_preceding_sibling(Tok, #xmlAttribute{}, Acc, Context) ->
    Acc;
match_preceding_sibling(Tok, #xmlNamespace{}, Acc, Context) ->
    Acc;
...
match_preceding_sibling(Tok, N, Acc, Context) ->
	...
	[#xmlNode{type = element,
		  node = #xmlElement{content = PContent}}|_] ->
	    PrecedingSiblings = lists:sublist(PContent, 1,
...
match_attribute(Tok, N, Acc, Context) ->
	...
	element ->
	    #xmlNode{parents = Ps, node = E, type = Type} = N,
	    lists:foldl(
	      fun(A, AccX) ->
...
node_test(F, N, Context) when function(F) ->
    F(N, Context);
node_test({wildcard, _}, N, Context) -> 
    true;
...
node_test({name, {Tag, Prefix, Local}}, 
	  N = #xmlNode{node = #xmlElement{name = Tag}}, Context) -> 
    %io:format("node_test({tag, ~p}, ~p) -> true.~n", [Tag, write_node(N)]),
    true;
node_test({name, {Tag, Prefix, Local}}, 
	  N = #xmlNode{node = #xmlElement{name = Name,
	...
	[] ->
	    ...
	    ?dbg("node_test(~p, ~p) -> ~p.~n", 
		 [{Tag, Prefix, Local}, write_node(N), Res]),
	    Res;
	ExpName ->
...
node_test({name, {Tag,Prefix,Local}}, 
	  #xmlNode{node = #xmlAttribute{name = Tag}}, Context) -> 
    true;
node_test({name, {Tag, Prefix, Local}}, 
	  N = #xmlNode{node = #xmlAttribute{expanded_name = {URI, Local},
					    nsinfo = {Prefix1, _},
					    namespace = NS}}, Context) -> 
...
node_test({node_type, NT}, #xmlNode{node = N}, Context) ->
    case {NT, N} of
	{text, {Data}} ->
	    true;
	{node, _} ->
...
node_test({processing_instruction, {literal, _, Name}}, 
	  #xmlNode{node = {processing_instruction, Name, Data}}, Context) ->
    true;
node_test(Other, N, Context) ->
    %io:format("node_test(~p, ~p) -> false.~n", [Other, write_node(N)]),
    false.
