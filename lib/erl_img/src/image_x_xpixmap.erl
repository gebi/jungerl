	...
	{ok,Bin} ->
		...
		true ->
			...
			{ok,Ts} ->
			    ...
			    {ok,[Width,Height,NColors,CharPerPixel],Rest} = 
...
tr_pixels([],Colors,PIX,NRows,NRows,RowFun,St0) ->
    {ok, PIX#erl_pixmap { pixels = St0 }};
tr_pixels(_, Colors, IMG, _, _, RowFun, St0) ->
    {error, bad_data}.
    

color_tab([{_, " c #"++Hex} | Cs]) ->
    Color = hex_to_integer(Hex),
    R = ((Color bsr 16) band 16#ff)*255,
    G = ((Color bsr 8) band 16#ff)*255,
    B = ((Color bsr 0) band 16#ff)*255,
    [ {R,G,B} | color_tab(Cs)];
...
%% parse the xpm file 
parse_xpm([{id,"static"},{id,"char"},'*',{id,Name},'[', ']','=', '{' | Ts]) ->
    parse_xpm(Ts, []);
parse_xpm(Ts) ->
    {error,{bad_format,Ts}}.
