...
glade_file() -> join([dirname(code:which(?MODULE)),gperf])++".glade".

loop(LD = #ld{name=Name}) ->
    receive
	{Name,{signal,{window,'GDK_DELETE'}}} -> die(LD);
	{Name,{signal,{But,'GDK_BUTTON_RELEASE'}}} -> loop(do_button(LD,But));
	{Name,{signal,{Darea,'GDK_EXPOSE'}}} -> loop(do_expose(LD,Darea));
...
snd_rec(Cmd, Args) ->
    GuiName = get(gui_name),
    GuiName ! {self(),[{Cmd,Args}]},
    receive
	{GuiName,{reply,[{ok,Rep}]}} -> 
	    %%io:fwrite("~w ~p -> ~p~n",[Cmd,Args,Rep]),
	    Rep;
	{GuiName,{reply,[{error,Rep}]}} -> erlang:fault({Cmd,Args,Rep})
    end.
